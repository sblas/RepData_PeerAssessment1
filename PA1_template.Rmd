...
The calculated mean number of steps taken per day is `r meandata` steps.  And the calculated median total number of steps taken per day is `r mediandata` steps.
...
Therefore, The `r maxsteps`th 5-minute interval, on average across all the days in the dataset for complete cases, contains the maximum number of steps.
...
The calculated mean number of steps taken per day for dataset with imputed data is `r meandata2` steps.  And the calculated median total number of steps taken per day for the dataset with imputed data is `r mediandata2`.

A summary of the steps per day from the first part of this assignment and from the imputed data can also be seen below.

```{r summaryboth, echo=TRUE}
